    ...
    def get_population(
        ...
        **args,
        ...
        population = Parallel(n_jobs=n_jobs)(
            delayed(cls.cast_neurondata)(path, **args)
    ...
    def to_python_hdf5(self, save_path: Path):
        ...
        with h5py.File(save_path, "w") as f:
                ...
                if key != "population":
                    if isinstance(value, np.ndarray):
                        ...
                        for dkey, dvalue in value.items():
                            if isinstance(dvalue, np.ndarray):
                                group.create_dataset(
                                    dkey, np.array(dvalue).shape, data=dvalue
                                )
                            else:
                                group.attrs[dkey] = dvalue
                    else:
            ...
            for ip, ipopulation in enumerate(self.population):
                ...
                save_keys = {
                    "code_samples": (ipopulation.code_samples, np.int32),
                    "code_numbers": (ipopulation.code_numbers, np.int16),
                    "sample_id": (ipopulation.sample_id, np.int8),
                    "test_distractor": (ipopulation.test_distractor, np.int8),
                    "test_stimuli": (ipopulation.test_stimuli, np.int8),
                }
    ...
    def execute_function(
        ...
        **kwargs,
        ...
        res = Parallel(n_jobs=n_jobs)(
            delayed(func)(neu, *args, **kwargs)
            for neu in tqdm(self.population)
        )
        if ret_df:
            # Check res is a list of dicts
            if isinstance(res, list) and all(
                isinstance(item, dict) for item in res
            ):
                df = pd.DataFrame(res)
                return df
